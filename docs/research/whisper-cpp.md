# whisper.cpp On-Device-Erkennung für WhisperBar — Wiederherstellung technisch abgesichert

Stand der Recherche: 2026-09-06 · Repo `/home/chris/whisperbar-android` (Branch `v3-redesign`, read-only) · Alt-Stand: Tag `offline-v1`
...
- **`app/src/main/cpp/CMakeLists.txt`**: `add_subdirectory(whisper.cpp)` mit `WHISPER_BUILD_TESTS/EXAMPLES/SERVER=OFF`, `WHISPER_SDL2/CURL=OFF`, `GGML_OPENMP=OFF`, `GGML_NATIVE=OFF`, `BUILD_SHARED_LIBS=OFF`; Target `whisperbar` (SHARED) linkt `log android whisper`. Solide Grundlage — bleibt.
...
- #3913 „Improved inference performance of Android example project“ — reiner Build-Fix: `-DCMAKE_BUILD_TYPE=Release` auch für AGP-Debug-Builds, weil AGP-Debug den Native-Build unoptimiert lässt („causing a critical performance issue“). **Für WhisperBar übernehmen (§8.3).**
...
**Entscheidung für WhisperBar (einfachster sicherer Weg):**
...
- **Ein Kontext prozessweit**: IME (`WhisperBarInputMethodService`), `FloatingMicService` und `ShareTranscribeActivity` laufen im selben App-Prozess → `WhisperEngine`-Singleton wie im Alt-Stand (`synchronized(lock)`, `ensureLoaded`), sonst läge das Modell doppelt im RAM.
...
set(BUILD_SHARED_LIBS      OFF CACHE BOOL "" FORCE)  # whisper+ggml statisch in libwhisperbar.so
...
target_link_libraries(whisperbar PRIVATE ${LOG_LIB} android whisper)

# Groesse/Geschwindigkeit wie examples/whisper.android (v1.9.3)
foreach(t whisperbar whisper ggml ggml-base ggml-cpu)
    if(TARGET ${t})
...
endforeach()
target_link_options(whisperbar PRIVATE
    -Wl,--gc-sections
    -Wl,--exclude-libs,ALL
...
+
+JNIEXPORT void JNICALL
+Java_com_chris_whisperbar_WhisperLib_requestAbort(JNIEnv *, jobject) { g_abort.store(true); }
+
+// Erkannte Sprache (bei language="auto"), z. B. "de" — fuer TextPolisher.
+JNIEXPORT jstring JNICALL
+Java_com_chris_whisperbar_WhisperLib_getDetectedLanguage(JNIEnv *env, jobject, jlong context_ptr) {
...
+JNIEXPORT void JNICALL
+Java_com_chris_whisperbar_WhisperLib_printTimings(JNIEnv *, jobject, jlong context_ptr) {
...
internal object WhisperLib {
    ...
    init { if (supported) System.loadLibrary("whisperbar") }
