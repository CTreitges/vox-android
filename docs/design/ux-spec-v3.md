...
# WhisperBar v3 — Verbindliche UX-Spezifikation
...
- Theme: fest dunkel (`darkColorScheme` aus §3), **kein** Dynamic Color, `enableEdgeToEdge(SystemBarStyle.dark(TRANSPARENT), SystemBarStyle.dark(TRANSPARENT))`, Manifest-Theme `Theme.Material.NoActionBar` mit `windowBackground = @color/wb_background`.
- Eine Farb-Wahrheit: alle Hex-Werte in `res/values/colors.xml` (Präfix `wb_`), Compose hebt sie per `colorResource()` in `darkColorScheme` + `LocalWbColors` (CompositionLocal für `recording`, `success`, `warning` + Container). IME/Overlay/Notification lesen dieselben `@color/wb_*`.
...
| E6 | Über WhisperBar | `ModalBottomSheet` | Version, Lizenzen, Quellcode |
...
│    title  "WhisperBar"                              titleLarge · onSurface
...
│    "Sprachnachrichten abtippen: in WhatsApp lange drücken → Teilen → WhisperBar."  · TextButton common_more "Mehr" → E5 Abschnitt 1
└─ FOOTER  Text bodySmall outline, zentriert: home_version "WhisperBar %1$s"  ·  TextButton(bodySmall) home_rerun_setup "Einrichtung erneut öffnen" → W Schritt 1 (Zurück = Home)
```
...
- bodyLarge `welcome_body` „WhisperBar nimmt auf, erkennt den Text über deinen eigenen Zugang oder ein Offline-Modell und tippt ihn ins aktuelle Feld. In ein paar Schritten ist alles bereit."
...
- Titel `setup_s1_title` „Wie soll WhisperBar Sprache erkennen?"; Text `setup_s1_body` „Du kannst später jederzeit wechseln."
...
- Titel `setup_s4_title` „Über anderen Apps anzeigen"; Text `setup_s4_body` „Der schwebende Mikro-Knopf liegt über anderen Apps. Dafür braucht Android deine Erlaubnis — du landest gleich in den Systemeinstellungen: dort „WhisperBar" einschalten und zurück."
- Aufklappbare `OutlinedCard` (TextButton `setup_more_info` „Was passiert dabei?" → AnimatedVisibility) mit 3 nummerierten Zeilen `setup_s4_step_1..3` („Systemeinstellung öffnet sich", „Schalter „Über anderen Apps anzeigen" einschalten", „Zurück-Taste — WhisperBar prüft automatisch").
...
- Aufklappbar „Was passiert dabei?": `setup_s5_step_1..3` („Bedienungshilfe-Einstellungen öffnen sich", „Installierte Apps → WhisperBar → Ein", „Bei „Eingeschränkte Einstellung": App-Info → ⋮ → Eingeschränkte Einstellungen zulassen, dann erneut"). Der dritte Punkt betrifft Sideload-Installationen ab Android 13 — im Gerätetest prüfen.
...
- Titel `setup_s7_title` „Diktat-Tastatur"; Text `setup_s7_body` „Alternative zum Knopf: die WhisperBar-Tastatur mit Halten-zum-Sprechen. Zwei Schritte: aktivieren, dann auswählen."
...
└─ ic_info               settings_group_about       "Über WhisperBar"  "Version {v}" → E6 (Sheet: Version, Lizenzen whisper.cpp MIT/Compose Apache 2.0, Link GitHub-Repo, Button common_close)
```
...
│    bodyMedium button_keyboard_intro "Alternative zum Knopf: WhisperBar als Tastatur mit Halten-zum-Sprechen. Nützlich, wenn eine App kein Overlay erlaubt."
...
8. Fußzeile bodySmall outline: `home_version` „WhisperBar %1$s" · `about_license` „whisper.cpp (MIT) · Jetpack Compose (Apache 2.0)" · Link `about_source` „Quellcode auf GitHub".
...
- **KEINE DATEI** (kein Audio-URI): ic_music_off outline · `share_no_audio` (bestehend) · `share_no_audio_body` „Teile eine Audiodatei oder Sprachnachricht mit WhisperBar." · Button `common_close`.
...
### 3.1 Farbschema (fest dunkel; `res/values/colors.xml` mit Präfix `wb_`, Compose `darkColorScheme` + `LocalWbColors`)
...
**Zusatz-Tokens** (`LocalWbColors` + `colors.xml`):
...
### 4.3 Notification-Small-Icon `drawable/ic_stat_whisperbar.xml` (24 × 24, reine Alpha-Silhouette, Weiß auf transparent; Quelle https://documentation.onesignal.com/docs/en/notification-icons)
...
- Sicherheitsrand 2 dp eingehalten (x 5,05–18,95; y 3–18). `Notification.Builder.setColor(wb_primary)`; während RECORDING `setColor(wb_recording)`.

### 4.4 Weitere Vektoren

`ic_mic` (24 dp, für Bubble/IME/Hero; Material-Symbol) · `ic_stop` · `ic_replay` · `ic_close` (Cancel-Ziel). Splash: keine Lib; Manifest-Theme mit `windowBackground = @color/wb_background` (Android 12+ zeigt System-Splash mit dem adaptiven Icon).
...
- Ebenen (hinten → vorn): `pulse_ring` (View 84 dp oval, Stroke 4, nur RECORDING) · `state_ring` (View 72 dp oval, Stroke 2) · `bubble` (68 dp oval, `bubble_bg.xml` mit `@color/wb_*`) · `bubble_progress` (`ProgressBar` indeterminate 76 dp, Stroke 3, `indeterminateTint wb_primary`, nur SENDING) · `bubble_icon` (`ImageView` 28 dp) · darunter (4 dp Abstand) `bubble_label` (`TextView` in Pille: Höhe 22, Radius 11, Padding 10/0, labelMedium 12 sp, Grund `wb_surfaceContainerLow` 92 % Alpha).
...
### 5.3 IME-Tastatur (`layout/keyboard_view.xml`, `WhisperBarInputMethodService`)

```
LinearLayout vertical, background @color/wb_background, oben 1 dp @color/wb_outlineVariant, paddingH 12, paddingTop 8, paddingBottom 12 (+ Nav-Inset); Gesamthöhe ≈ 240 dp
...
│                (Attack 50 ms / Release 250 ms); Farbe wb_recording bei Aufnahme, sonst wb_surfaceContainerHighest flach; importantForAccessibility = no
...
│                IDLE      wb_surfaceContainerHigh + 2 dp Ring wb_primary, ic_mic 36 dp wb_primary
│                RECORDING wb_recording, ic_stop 36 dp wb_onRecording, Puls
│                SENDING   wb_primaryContainer + ProgressBar-Bogen wb_primary, ic_mic wb_onPrimaryContainer @ 50 %
│                ERROR     wb_errorContainer + 2 dp Ring wb_error, ic_replay wb_onErrorContainer
│                contentDescription cd_mic (bestehend); Halten-zum-Sprechen bleibt (ACTION_DOWN/UP)
└─ Tastenreihe   LinearLayout 52 dp, marginTop 8, Abstand 4 dp; Tasten: Höhe 48, minWidth 44, Radius 12, Füllung wb_surfaceContainerHigh, gedrückt wb_surfaceContainerHighest,
                 Ripple wb_onSurface @ 12 %; Icons 22 dp wb_onSurface, Text 18 sp wb_onSurface
                 ...
                 [ic_replay cd_kb_retry, Farbe wb_error — nur ERROR sichtbar] [ic_settings cd_kb_settings → MainActivity route=settings]
```

Subtypes de/en (`method.xml`) bleiben. Tastenhöhe wächst ab `fontScale ≥ 1,3` auf 56 dp. Alle Drawables referenzieren ausschließlich `@color/wb_*`.
...
`setSmallIcon(ic_stat_whisperbar)`, `setColor(wb_primary)` (RECORDING: `wb_recording`), Titel `float_running`, Text `float_running_text`, Aktion `float_stop` „Beenden" (`ic_stop`), `setOngoing(true)`, Kanal `float_channel` `IMPORTANCE_LOW`, **`contentIntent` → `MainActivity(route=home)`** (neu — bisher totes Ende).
...
| Key | Text |
|---|---|
| `app_name` (bestehend) | WhisperBar |
| `ime_label` (bestehend) | WhisperBar Diktat |
...
| `home_share_hint` | Sprachnachrichten abtippen: in WhatsApp lange drücken → Teilen → WhisperBar. |
| `home_rerun_setup` | Einrichtung erneut öffnen |
| `home_version` | WhisperBar %1$s |
...
| `welcome_body` | WhisperBar nimmt auf, erkennt den Text über deinen eigenen Zugang oder ein Offline-Modell und tippt ihn ins aktuelle Feld. In ein paar Schritten ist alles bereit. |
...
| `setup_s1_title` | Wie soll WhisperBar Sprache erkennen? |
...
| `setup_s4_body` | Der schwebende Mikro-Knopf liegt über anderen Apps. Dafür braucht Android deine Erlaubnis — du landest gleich in den Systemeinstellungen: dort „WhisperBar" einschalten und zurück. |
| `setup_s4_step_1` | Die Systemeinstellung öffnet sich |
| `setup_s4_step_2` | Schalter „Über anderen Apps anzeigen" einschalten |
| `setup_s4_step_3` | Zurück-Taste — WhisperBar prüft automatisch |
...
| `setup_s5_step_2` | Installierte Apps → WhisperBar → Ein |
...
| `setup_s7_body` | Alternative zum Knopf: die WhisperBar-Tastatur mit Halten-zum-Sprechen. Zwei Schritte: aktivieren, dann auswählen. |
...
| `a11y_label` (bestehend) | WhisperBar Text-Einfügen |
...
| `settings_group_about` | Über WhisperBar |
...
| `button_keyboard_intro` | Alternative zum Knopf: WhisperBar als Tastatur mit Halten-zum-Sprechen. Nützlich, wenn eine App kein Overlay erlaubt. |
...
| `help_s1_share` | Sprachnachrichten: in WhatsApp lange drücken → Teilen → WhisperBar. Der Text erscheint in Absätzen, du kannst ihn kopieren oder weitergeben. |
...
| `help_s6_online` | Online: Audio und Kontext-Prompt gehen an den gewählten Anbieter. Bei Textverbesserung geht der erkannte Text an das Sprachmodell. Dein Key bleibt auf dem Gerät. WhisperBar speichert keine Aufnahmen. |
...
| `help_p1_body` | „Über anderen Apps anzeigen" muss erlaubt sein. Zusätzlich in den Android-Einstellungen die Akku-Optimierung für WhisperBar ausschalten. |
| `help_p2` | Text landet nur in der Zwischenablage |
| `help_p2_body` | Die Bedienungshilfe „WhisperBar" ist aus. Aktivieren, dann fügt WhisperBar den Text direkt ein. |
...
| Key | Text |
|---|---|
| `share_label` (bestehend) | Mit WhisperBar transkribieren |
...
| `share_no_audio_body` | Teile eine Audiodatei oder Sprachnachricht mit WhisperBar. |
...
| Key | Text |
|---|---|
| `float_channel` (bestehend) | WhisperBar Diktat |
| `float_running` (bestehend) | WhisperBar-Diktat aktiv |
...
| `float_not_configured` | Kein Zugang eingerichtet — in WhisperBar einrichten |
...
| `cd_kb_settings` | WhisperBar-Einstellungen |
...
6. Die Activity trägt im Teilen-Menü das Icon `ic_share` (Mikrofon + Textzeilen) und das Label „Mit WhisperBar transkribieren".
...
5. Alle Farben stammen aus `@color/wb_*`; kein zweiter Hex-Wert in Drawables/Code.

**V2 IME-Tastatur**
1. Grund `wb_background`, Tasten `wb_surfaceContainerHigh`, Text/Icon `wb_onSurface`; keine Emoji-Glyphen.
...
**N1 Notification**
1. Small-Icon ist die weiße Silhouette `ic_stat_whisperbar` (kein Quadrat); Farbe `wb_primary`, während RECORDING `wb_recording`.
...
| `WhisperBarInputMethodService` (Status, Pegel, Halten, Tasten, Retry, Zahnrad, Subtypes) | V2 |
