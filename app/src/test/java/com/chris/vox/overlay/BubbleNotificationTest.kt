...
class BubbleNotificationTest {
    ...
    @Test fun smallIconIstDieSilhouette() {
        ...
        assertEquals(R.drawable.ic_stat_vox, n.smallIcon.resId)
    }
    ...
    @Test fun farbeFolgtDerAufnahme() {
        assertEquals(ctx.getColor(R.color.vox_primary), BubbleNotification.build(ctx, BubbleState.IDLE).color)
        assertEquals(ctx.getColor(R.color.vox_primary), BubbleNotification.build(ctx, BubbleState.SENDING).color)
        assertEquals(ctx.getColor(R.color.vox_recording), BubbleNotification.build(ctx, BubbleState.RECORDING).color)
    }
}
