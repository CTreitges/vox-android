...
class MainFlowTest {
    ...
    private fun app(env: AppEnv) {
        compose.setContent { VoxTheme { VoxApp(env) } }
        compose.waitForIdle()
    }
    ...
    private fun screen(env: AppEnv, content: @Composable (NavState) -> Unit) {
        ...
        compose.setContent {
            VoxTheme { CompositionLocalProvider(LocalAppEnv provides env) { content(nav) } }
        }
        compose.waitForIdle()
    }
    ...
    @Test fun routerZeigtSchritt1WennEngineFehlt() {
        ...
        app(env())
        compose.onNodeWithText("Wie soll Vox Sprache erkennen?").assertIsDisplayed()
    ...
    @Test fun hubZeigtSechsZeilen() {
        ...
        listOf("Erkennung", "Text", "Knopf & Tastatur", "Offline-Modelle", "Anleitung & Hilfe", "Über Vox")
            .forEach { compose.onNodeWithText(it).assertExists() }
