package com.chris.vox.ui.state
...
import com.chris.vox.api.ApiAccess
...
class PrefsState(val prefs: Prefs) {
    ...
    private fun <T> pref(initial: T, write: (T) -> Unit): ReadWriteProperty<Any?, T> = PrefField(initial, write)

    private class PrefField<T>(initial: T, private val write: (T) -> Unit) : ReadWriteProperty<Any?, T> {
