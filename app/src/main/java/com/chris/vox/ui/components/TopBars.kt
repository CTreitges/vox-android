...
fun SmallTopBar(
    ...
    TopAppBar(
        ...
        navigationIcon = {
            if (navIcon != null) {
                IconButton(onClick = onNav) { VoxIcon(navIcon, navContentDescription) }
            }
        },
        actions = actions,
    ...
    Scaffold(
        ...
        topBar = {
            LargeTopAppBar(
                title = { Text(title) },
                navigationIcon = {
                    IconButton(onClick = onBack) { VoxIcon(R.drawable.ic_arrow_back, stringResource(R.string.cd_back)) }
                },
