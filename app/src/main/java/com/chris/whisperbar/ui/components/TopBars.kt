...
fun SmallTopBar(
    ...
    TopAppBar(
        ...
        navigationIcon = {
            if (navIcon != null) {
                IconButton(onClick = onNav) { WbIcon(navIcon, navContentDescription) }
            }
        },
        actions = actions,
    ...
    Scaffold(
        ...
        topBar = {
            LargeTopAppBar(
                title = { Text(title) },
                navigationIcon = {
                    IconButton(onClick = onBack) { WbIcon(R.drawable.ic_arrow_back, stringResource(R.string.cd_back)) }
                },
