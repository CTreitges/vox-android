...
fun KeySheet(providers: List<Provider>, snack: SnackController, onDismiss: () -> Unit) {
    WbSheet(title = stringResource(R.string.key_sheet_title), onDismiss = onDismiss) { dismiss ->
        Column {
...
fun CustomModelSheet(
    ...
    WbSheet(title = stringResource(R.string.model_custom_title), onDismiss = onDismiss) { dismiss ->
        val apply = {
            if (text.isNotBlank()) {
                onApply(text.trim())
                dismiss()
