...
import com.chris.whisperbar.PolishPlan
...
fun FillersSheet(onDismiss: () -> Unit) {
    ...
    WbSheet(title = stringResource(R.string.fillers_title), onDismiss = onDismiss) { dismiss ->
            ...
            FlowRow(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                prefs.customFillers.sorted().forEach { word ->
                    InputChip(
                        ...
                        label = { Text(word) },
                        trailingIcon = {
                            WbIcon(R.drawable.ic_close, stringResource(R.string.cd_remove_word, word), Modifier.size(18.dp))
                        },
                    )
                }
            }
        ...
        OutlinedTextField(
            ...
            trailingIcon = {
                IconButton(onClick = { addWords() }) {
                    WbIcon(R.drawable.ic_add, stringResource(R.string.cd_add_word))
                }
            },
        )
        Text(
