 ...
 * Small-Icon, Akzentfarbe wb_primary (waehrend RECORDING wb_recording), Aktion "Beenden",
 * Tipp oeffnet Home. Als eigenes Objekt, damit der Builder ohne laufenden Service testbar ist.
 */
object BubbleNotification {

    const val ID = 42
    const val CHANNEL_ID = "whisperbar_float"
    ...
    fun build(ctx: Context, state: BubbleState): Notification {
        ...
        val color = ctx.getColor(if (state == BubbleState.RECORDING) R.color.wb_recording else R.color.wb_primary)
