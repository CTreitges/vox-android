...
class BubbleRenderer(root: View, private val reduceMotion: () -> Boolean) {
        ...
        label.setTextColor(
            ctx.getColor(
                when (visual.labelStyle) {
                    BubbleVisual.LabelStyle.NEUTRAL -> R.color.wb_onSurfaceVariant
                    BubbleVisual.LabelStyle.RECORDING -> R.color.wb_recordingText
                    BubbleVisual.LabelStyle.ERROR -> R.color.wb_onErrorContainer
                },
            ),
        )
    ...
    private fun timerSpan(elapsedMs: Long): CharSequence {
        val dotColor = if (BubbleUi.dotVisible(elapsedMs)) ctx.getColor(R.color.wb_recording) else Color.TRANSPARENT
