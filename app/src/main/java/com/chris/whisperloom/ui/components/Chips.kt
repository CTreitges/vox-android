...
fun StatusChip(
    ...
    Row(
        ...
        LoomIcon(icon, null, Modifier.size(18.dp), onContainer)
...
fun StepStateChip(state: StepState, modifier: Modifier = Modifier) {
    ...
    when (state) {
        StepState.DONE -> StatusChip(
            ...
            R.drawable.ic_check_circle, loom.successContainer, loom.onSuccessContainer, modifier,
        )
        StepState.OPEN -> StatusChip(
            androidx.compose.ui.res.stringResource(R.string.setup_chip_open),
            R.drawable.ic_warning, loom.warningContainer, loom.onWarningContainer, modifier,
        )
